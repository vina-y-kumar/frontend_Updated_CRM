...
const InstagramPost = ({ uploadedImageUrl }) => {
  ...
  const addFiles = async (newFiles, urls = []) => {
    // Check if total files exceed the limit
    if (files.length + newFiles.length + urls.length > 10) {
      alert("You can only upload up to 10 files.");
      return;
    }
    ...
    const urlFiles = await Promise.all(
      urls.map(async (url, index) => {
        ...
        return await urlToFile(url, `image_${index + 1}`, mimeType);
      })
    );

    // Update state with new files
    setFiles([...files, ...validFiles, ...urlFiles]);
  };
...
>
  Post
    ...
    <span className="preview-username">nuren.ai</span>
      ...
      {/* Preview for external image URL */}
      {editorimageurl && (
        <div className="preview-media">
          <img src={editorimageurl} alt="External Image Preview" />
        </div>
      )}
